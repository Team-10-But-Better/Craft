...
/// The following struct adds a way to store the status of the player:
/// health: the current measure of how close the player is to death
/// hunger: the current measure of how starved the player is
/// enchant: the current measure of how much experience the player has gained from killing monsters
typedef struct
{
    double health;
    double hunger;
...
// Defining a SurvivalStatus to be used on a global scale
SurvivalStatus playerStatus;
...
/// As a part of [Issue #59]:https://github.com/Team-10-But-Better/Craft/issues/59
/// The following function initializes the player's SurvivalStatus
void initializeSurvivalStatus()
{
    playerStatus.health = 10;
    playerStatus.hunger = 10;
    playerStatus.enchant = 0;
}
...
/// As a part of [Issue #59]:https://github.com/Team-10-But-Better/Craft/issues/59
/// This method subtracts the amount of fall damage taken from the player's health
void takeFallDamage(int damage)
{
    ...
    printf("Current player health: %f\n", round(playerStatus.health));
}
...
int craft_main(int argc, char **argv)
{
    ...
    while (running)
    {
        ...
        /// As a part of [Issue #59]:https://github.com/Team-10-But-Better/Craft/issues/59
        /// Initialize the player's SurvivalStatus
        initializeSurvivalStatus();

        /// As a part of [Issue #59]:https://github.com/Team-10-But-Better/Craft/issues/59
        /// The following three variables keep track of a player's "falling state" that
        /// determines whether a player will take fall damage or not
        int falling = 0;
        int fallCount = 0;
        int fallDamage = 0;
